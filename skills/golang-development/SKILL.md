...
description: Guidelines and rules for golang development. Always use when user asks to start a project in go, or when working on an existing golang codebase.
---
...
## Code Shape and Engineering Style

Prefer straightforward Go over framework-shaped code. Optimize for locality, readable control flow, and small diffs. Keep names and abstraction count low unless they pay for themselves immediately.

- Keep logic close to where it is used. Do not spread a simple change across many files, helpers, or types unless the domain boundary requires it.
- Add top-level helper functions only when they hide real mechanics, are reused, or make the call site materially clearer.
- Prefer local variables, local closures, and inline structs for one-off setup over new package-level symbols.
- Avoid unnecessary type/symbol noise. Do not introduce manager/coordinator/result/config structs just to organize a small amount of code.
- Prefer clear inline code plus short comments over extracting small fragments into helpers that force the reader to jump around.
- Use comments to explain non-obvious decisions, invariants, constraints, or tradeoffs. Do not comment code that is already self-explanatory.
- Exported symbols need useful doc comments that describe behavior and contracts. Unexported helpers usually do not need comments unless they encode subtle logic.
- Make the minimal correct change. Avoid opportunistic rewrites, broad renames, churn, or style-only changes outside the task.
- Preserve existing patterns unless they are part of the problem. Match the surrounding code before introducing a new style.
- Prefer explicit, boring control flow over clever generic abstractions. Readability at the call site matters more than reducing a few repeated lines.
