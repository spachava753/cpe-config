...
- The shared compaction handoff is the next session's first user message, not retained prior messages. History remains available through `acp.star`. Keep the tool schema and template aligned: `currentTask`, `constraints` (which may be empty), `summary`, and `nextAction` are required; the template intentionally does not insert the original conversation root.
